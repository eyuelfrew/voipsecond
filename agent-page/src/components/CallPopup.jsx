...
import { stopIncomingCallRingtone } from "../utils/ringtone";
...
const CallPopup = ({
    ...
    useEffect(() => {
        if (incomingCall) {
            console.log('📞 CallPopup: Setting activeView to incoming');
        ...
        console.log('📞 CallPopup: Current activeView:', activeView);
    ...
    // Extract remote number with multiple fallbacks
    const remoteNumber = isIncoming
        ? (incomingCall?.remote_identity?.uri?.user ||
            incomingCall?.remoteIdentity?.uri?.user ||
            incomingCall?._remote_identity?.uri?.user ||
            incomingCall?.request?.from?.uri?.user ||
            incomingCall?.from?.uri?.user ||
            'Unknown')
    ...
    // Debug logging for screen pop
    useEffect(() => {
        if (isIncoming) {
            console.log('📞 CallPopup: Incoming call detected');
        }
    }, [isIncoming, incomingCall, remoteNumber]);
            ...
            <div className="fixed right-6 top-24 z-50 animate-slide-in-right">
                <div className="bg-white rounded-2xl shadow-2xl border-2 border-gray-200 p-4 w-80">
                    ...
                    <div className="flex items-center space-x-2">
                        <button
                            onClick={handleMute}
                            className={`flex-1 flex items-center justify-center space-x-1 py-2 rounded-lg font-semibold transition-all ${isMuted ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                }`}
                        >
                        ...
                        <button
                            onClick={handleHold}
                            className={`flex-1 flex items-center justify-center space-x-1 py-2 rounded-lg font-semibold transition-all ${isHeld ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                }`}
                        >
                                ...
                                <div className="relative">
                                    ...
                                    <div className="bg-gray-50 px-6 py-3 flex items-center justify-between border-b border-gray-200">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            {/* Status Indicator */}
                                            <div className="flex items-center space-x-2">
                                                <div className={`w-2 h-2 rounded-full ${agentStatus === 'Available' ? 'bg-green-500 animate-pulse' :
                                        ...
                                        {activeView === 'call' && hasActiveCall && (
                                            <div className="flex flex-col h-full space-y-8 animate-slide-up">
                                                    ...
                                                    <div className="flex items-center space-x-3 bg-white rounded-xl px-5 py-3 border border-gray-200 shadow-sm">
                                                        ...
                                                        <div className="flex items-center space-x-1">
                                                            {[...Array(5)].map((_, i) => (
                                                                <div
                                                                    key={i}
                                                                    className={`w-1 h-${Math.min(8, Math.floor((i + 1) * 1.6))} rounded-full transition-all duration-150 ${audioLevel > (i * 20)
                                                                        ? 'bg-green-500'
                                                                        : 'bg-gray-300'
                                                                        }`}
                                                                    style={{
                                                ...
                                                <div className="grid grid-cols-3 gap-3 mt-auto">
                                                    <button
                                                        onClick={handleMute}
                                                        className={`flex flex-col items-center justify-center space-y-2 p-4 rounded-2xl transition-all border ${isMuted
                                                    ...
                                                    <button
                                                        onClick={handleHold}
                                                        className={`flex flex-col items-center justify-center space-y-2 p-4 rounded-2xl transition-all border ${isHeld
                                                    ...
                                                    <button
                                                        ...
                                                        className={`flex flex-col items-center justify-center space-y-2 p-4 rounded-2xl transition-all border ${showTransfer
