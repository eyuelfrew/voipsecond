  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isEditMode && id) {
        ...
        if (response.data) {
          showCustomAlert('IVR Menu updated successfully!', 'success');
          setTimeout(() => navigate('/ivr-menus'), 1500);
        }
      } else {
        // Create new IVR
        ...
        if (response.data) {
          showCustomAlert('IVR Menu created successfully!', 'success');
          setTimeout(() => navigate('/ivr-menus'), 1500);
        }
      }
    } catch (error) {
      console.error('Error saving IVR menu:', error);
      showCustomAlert(`Failed to ${isEditMode ? 'update' : 'create'} IVR menu.`, 'error');
    } finally {
      setSubmitting(false);
    }
  };
