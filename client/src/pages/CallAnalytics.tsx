...
type CallQualityMetric = {
  mosScore?: number;
  jitter?: number;
  packetLoss?: number;
  rtt?: number;
  callQualityRating?: string;
  qualityScore?: number;
...
const qualityRatingClasses: Record<string, string> = {
  excellent: 'bg-green-100 text-green-800 ring-1 ring-inset ring-green-200',
  good: 'bg-blue-100 text-blue-800 ring-1 ring-inset ring-blue-200',
  fair: 'bg-yellow-100 text-yellow-800 ring-1 ring-inset ring-yellow-200',
  poor: 'bg-orange-100 text-orange-800 ring-1 ring-inset ring-orange-200',
  bad: 'bg-red-100 text-red-800 ring-1 ring-inset ring-red-200',
};

type Filters = { from: string; to: string; callerId: string; callee: string; onlyWithRecordings: '' | 'true' | 'false'; queueName: string; agentExtension: string; qualityRating: string };
...
const CallAnalytics: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'startTime' | 'duration' | 'status' | 'callerId' | 'callee' | 'qualityScore'>('startTime');
  ...
  useEffect(() => {
    const saved = localStorage.getItem('callAnalytics.pageSize');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('callAnalytics.pageSize', String(pageSize));
  }, [pageSize]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (filters.agentExtension) params.agentExtension = filters.agentExtension;
      if (filters.qualityRating) params.qualityRating = filters.qualityRating;
      if (filters.onlyWithRecordings === 'true' || filters.onlyWithRecordings === 'false') params.hasRecording = filters.onlyWithRecordings;
      if (q) params.q = q;
      const res = await axios.get(`${baseUrl}/api/report/call-analytics`, { params });
  ...
  const qualityRatingToBadge = (rating?: string) => (rating ? qualityRatingClasses[rating] || 'bg-slate-50 text-slate-700 ring-1 ring-inset ring-slate-200' : 'bg-slate-50 text-slate-700 ring-1 ring-inset ring-slate-200');
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      <header className="mb-3 sm:mb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Browse and analyze call recordings with quality metrics. Use filters, sort, and pagination.</p>
          </div>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur border border-gray-200 p-4 sm:p-5 rounded-xl shadow-sm mb-4">
        <div className="flex flex-wrap items-end gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-8 gap-4 flex-1">
            <div>
              <label className="block text-sm font-medium">From</label>
              <input className="border rounded px-2 py-1 w-full" type="date" name="from" value={filters.from} onChange={handleFilterChange} />
            </div>
            <div>
              <label className="block text-sm font-medium">To</label>
              <input className="border rounded px-2 py-1 w-full" type="date" name="to" value={filters.to} onChange={handleFilterChange} />
            </div>
            <div>
              <label className="block text-sm font-medium">Caller ID</label>
              <input className="border rounded px-2 py-1 w-full" type="text" name="callerId" value={filters.callerId} onChange={handleFilterChange} />
            </div>
            <div>
              <label className="block text-sm font-medium">Callee</label>
              <input className="border rounded px-2 py-1 w-full" type="text" name="callee" value={filters.callee} onChange={handleFilterChange} />
            </div>
            <div>
              <label className="block text-sm font-medium">Queue</label>
              <input className="border rounded px-2 py-1 w-full" type="text" name="queueName" value={filters.queueName} onChange={handleFilterChange} placeholder="Queue name" />
            </div>
            <div>
              <label className="block text-sm font-medium">Agent</label>
              <input className="border rounded px-2 py-1 w-full" type="text" name="agentExtension" value={filters.agentExtension} onChange={handleFilterChange} placeholder="Agent extension" />
            </div>
            <div>
              <label className="block text-sm font-medium">Quality</label>
              <select className="border rounded px-2 py-1 w-full" name="qualityRating" value={filters.qualityRating} onChange={handleFilterChange}>
            ...
            <div>
              ...
              <select className="border rounded px-2 py-1 w-full" name="onlyWithRecordings" value={filters.onlyWithRecordings} onChange={handleFilterChange}>
        ...
        {loading ? (
          ...
          <table className="min-w-full">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                {[{ k: 'callerId', t: 'Caller ID' }, { k: 'callerName', t: 'Caller Name' }, { k: 'callee', t: 'Callee' }, { k: 'queueName', t: 'Queue' }, { k: 'agentName', t: 'Agent' }, { k: 'status', t: 'Status' }, { k: 'qualityScore', t: 'Quality' }, { k: 'startTime', t: 'Start' }, { k: 'endTime', t: 'End' }, { k: 'duration', t: 'Duration' }, { k: 'rec', t: 'Recording' }].map((col) => (
                  <th key={col.k} className="px-4 py-3 text-left text-xs font-semibold text-gray-600 tracking-wider">
                    <button
                      type="button"
                      className="flex items-center gap-1 hover:text-gray-900"
                      onClick={() => {
                        if (col.k === 'rec' || col.k === 'callerName' || col.k === 'endTime' || col.k === 'queueName' || col.k === 'agentName') return;
                        const nextOrder = (sortBy === (col.k as any) ? (sortOrder === 'asc' ? 'desc' : 'asc') : 'asc');
                        if (col.k === 'callerId' || col.k === 'callee' || col.k === 'status' || col.k === 'duration' || col.k === 'startTime' || col.k === 'qualityScore') {
              ...
              {items.length === 0 ? (
                ...
                items.map((r, idx) => (
                  <tr key={r.id} className={idx % 2 === 0 ? 'bg-white hover:bg-slate-50' : 'bg-slate-50/50 hover:bg-slate-100'}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{r.queueName || '-'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{r.agentName || '-'}</td>
                    ...
                    <td className="px-4 py-3 text-sm">
                      {r.qualityMetrics ? (
                        <div className="flex flex-col">
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${qualityRatingToBadge(r.qualityMetrics.callQualityRating)}`}>
                            {r.qualityMetrics.callQualityRating || '-'}
                          </span>
                          <div className="text-[10px] text-gray-500 mt-1">
                            {r.qualityMetrics.mosScore && <span>MOS: {r.qualityMetrics.mosScore.toFixed(1)}</span>}
                            {r.qualityMetrics.packetLoss && r.qualityMetrics.packetLoss > 0 && (
                              <span className="ml-1">PL: {r.qualityMetrics.packetLoss.toFixed(2)}%</span>
                            )}
                          </div>
                        </div>
                      ) : (
          ...
          <div className="relative bg-white rounded-xl shadow-xl border border-gray-200 w-[92vw] max-w-2xl mx-auto p-5">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                {listenItem.qualityMetrics && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    <span className={`text-xs px-2 py-1 rounded-full ${qualityRatingToBadge(listenItem.qualityMetrics.callQualityRating)}`}>
                      Quality: {listenItem.qualityMetrics.callQualityRating || 'N/A'}
                    </span>
                    {listenItem.qualityMetrics.mosScore && (
                      <span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800">
                        MOS: {listenItem.qualityMetrics.mosScore.toFixed(2)}
                      </span>
                    )}
                    {listenItem.qualityMetrics.packetLoss && (
                      <span className="text-xs px-2 py-1 rounded-full bg-amber-100 text-amber-800">
                        Packet Loss: {listenItem.qualityMetrics.packetLoss?.toFixed(2)}%
                      </span>
                    )}
                    {listenItem.qualityMetrics.jitter && (
                      <span className="text-xs px-2 py-1 rounded-full bg-indigo-100 text-indigo-800">
                        Jitter: {listenItem.qualityMetrics.jitter?.toFixed(2)}ms
                      </span>
                    )}
                  </div>
                )}
