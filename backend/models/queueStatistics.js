...
const queueStatisticsSchema = new mongoose.Schema({
  ...
  hourlyStats: {
    ...
    of: {
      ...
      totalTalkTime: { type: Number, default: 0 },
...
queueStatisticsSchema.statics.getHourlyTrends = async function (queueId, date) {
  ...
  if (!stats || !stats.hourlyStats) return [];
  
  // Convert Map to array format for easier consumption
  const hourlyArray = [];
  for (let hour = 0; hour < 24; hour++) {
    const hourKey = hour.toString();
    const data = stats.hourlyStats.get(hourKey) || {
      calls: 0,
      answered: 0,
      abandoned: 0,
      avgWaitTime: 0,
